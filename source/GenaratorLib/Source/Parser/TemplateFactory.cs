	...
	static public class TemplateFactory
	{
		...
		static readonly Dictionary<NsTypes,string> AC001 = new Dictionary<NsTypes,string>(){
			...
			{ NsTypes.TableTypes, string.Concat(
				...
				Strings.Factory_AcPrime) },
			{ NsTypes.AdapterTypes,
				Strings.Factory_AcAdapt },
			{ NsTypes.DatabaseTypes,
				Strings.Factory_AcData },
			{ NsTypes.FieldTypes, string.Concat(
				Strings.Factory_AcField,";",
		...
		static readonly Regex regex_fieldTemplateTag = new Regex(Strings.Regex_Field_Template_Tag,RegexOptions.Multiline);
		                          ...
		                          bool ignoreException=true)
		{
			...
			if (hasError = (db==null || table==null))
			{
				if (showMessageBox) System.Windows.Forms.MessageBox.Show(
					Strings.MsgDatabaseOrTableNullError_Title,
					Strings.MsgDatabaseOrTableNullError_Message
				);
				if (!ignoreException) throw new ArgumentException (Strings.MsgDatabaseOrTableNullError_Exception);
			}
			return hasError;
		}
		...
		static public string ConvertInput(Generator.Export.Intrinsic.IDbConfiguration4 tps, bool newVersion)
		{
			...
			else if (tps.SelectedView!=null)
			{
				...
				{
					return Strings.MsgDatabaseOrTableNullError_Exception;
				}
...
//			QuickMatch match0 = list[0];
			...
			if (!match.HasMultipleParams)
			{
				...
				if (tbltmpl==null) {
					Logger.Warn(
						Strings.LogTemplateFactoryErr_Title,
						Strings.LogTemplateFactoryErr_Filter,
						match. Name
					);
					return tableOut;
				}
